...
-(HeadImageView *)headImageView{
    if (!_headImageView) {
       ...
       //VIP图标
        ...
        if ([[NSUSE_DEFO objectForKey:@"username"] isEqualToString:@"15032735032"] || [ToolClass isLogin]==NO) {
            vipimage.hidden=YES;
            renImage.hidden=YES;
        }
...
-(void)createNav{
    ...
    if ([[NSUSE_DEFO objectForKey:@"username"] isEqualToString:@"15032735032"]) {
       [self.NavView.rightBtn setTitle:@"投诉" forState:0];
        [self.NavView.rightBtn addTarget:self action:@selector(juBaoBtn) forControlEvents:UIControlEventTouchUpInside];
    }else{
...
-(void)delegateBtn:(UIButton*)btn{
   WhoYanYuanModel*md= _dongTaiArr[btn.tag];
    ...
    UIAlertController * actionView =[UIAlertController alertControllerWithTitle:@"" message:@"删除动态" preferredStyle:UIAlertControllerStyleAlert];
...
//    
//    if ([_phoneNum isEqualToString:[NSUSE_DEFO objectForKey:@"username"]]) {
//        [Engine upDataYinCangStypesuccess:^(NSDictionary *dic) {
//            NSString * content =[ToolClass isString:[NSString stringWithFormat:@"%@",[dic objectForKey:@"content"]]];
//            if ([content isEqualToString:@"0"]) {
//                //隐藏
//                NSLog(@"这是隐藏状态");
//                JiBenMessageCell * cell =[_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
//               // cell.swit.on=YES;
//                cell.nameLabel.text=@"*****";
//            }else{
//                //显示
//                 NSLog(@"这是打开状态");
//                JiBenMessageCell * cell =[_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
//                cell.nameLabel.text=_jibenArr[0][2];
//               // cell.swit.on=NO;
//            }
//        } error:^(NSError *error) {
//            
...
-(void)switchAction:(UISwitch*)swit{
        ...
        [Engine lianxiStypesuccess:^(NSDictionary *dic) {
            NSString * content =[ToolClass isString:[NSString stringWithFormat:@"%@",[dic objectForKey:@"content"]]];
            if ([content isEqualToString:@"0"]) {
                //隐藏
                JiBenMessageCell * cell =[_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0]];
...
/*
 ...
 NSString * show =[ToolClass isString:[NSString stringWithFormat:@"%@",[dicc objectForKey:@"show_connecttel"]]];
