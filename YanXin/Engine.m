...
#pragma mark --26获取实名认证资料
+(void)huoQuShiMingRenZhengZiLiaosuccess:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/user/app_getUserAuthentication.action",SER_VICE];
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    NSString * token =[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]];
    
    if (token==nil) {
        [LCProgressHUD showMessage:@"26获取实名认证资料"];
        return;
    }else{
        [dic setObject:token forKey:@"account"];
    }
    ...
    [manager POST:urlStr parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *data = [NSJSONSerialization dataWithJSONObject:responseObject options:NSJSONWritingPrettyPrinted error:nil];
        NSString *str = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"26获取实名认证资料%@",str);
        
        aSuccess(responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        
        NSLog(@"26获取实名认证资料%@",error);
    }];
...
#pragma mark --27上传图片
+(void)ShangChuanImageStr:(NSString*)imageStr success:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/actor/app_uploadImgInHomepage.action",SER_VICE];
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    NSString * token =[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]];
    
    if (token==nil) {
        [LCProgressHUD showMessage:@"27上传图片"];
        return;
    }else{
        [dic setObject:token forKey:@"account"];
    }
    
    [dic setObject:imageStr forKey:@"imgArr"];
...
#pragma mark --28上传视频
+(void)shangChuanVivo:(NSString*)vivoArr success:(SuccessBlock)aSuccess error:(ErrorBlock)aError
{
    
    NSString * urlStr =[NSString stringWithFormat:@"%@/actor/app_uploadVideoInHomepage.action",SER_VICE];
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    NSString * token =[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]];
    
    if (token==nil) {
        [LCProgressHUD showMessage:@"28没有登录"];
        return;
    }else{
        [dic setObject:token forKey:@"account"];
    }

    [dic setObject:vivoArr forKey:@"videoArr"];
...
#pragma mark --29删除图片或者视频
+(void)deleteImageOrVivoID:(NSString*)idd success:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/actor/app_deleteMultimediaInAlbum.action",SER_VICE];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    NSString * token =[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]];
    if (token) {
         [dic setObject:token forKey:@"account"];
    }else{
        [LCProgressHUD showMessage:@"29没有登录"];
        return;
    }
    [dic setObject:idd forKey:@"media_id"];
    
    AFHTTPRequestOperationManager * manager =[AFHTTPRequestOperationManager manager];
    [manager POST:urlStr parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *data = [NSJSONSerialization dataWithJSONObject:responseObject options:NSJSONWritingPrettyPrinted error:nil];
        NSString *str = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"29删除图片或者视频%@",str);
        
        aSuccess(responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        
        NSLog(@"29删除图片或者视频%@",error);
    }];
}
...
#pragma mark--30切换联系方式可见状态
+(void)lianxiStypesuccess:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/user/app_toggleShowConnecttel.action",SER_VICE];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    [dic setObject:[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]] forKey:@"account"];
    ...
    [manager POST:urlStr parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *data = [NSJSONSerialization dataWithJSONObject:responseObject options:NSJSONWritingPrettyPrinted error:nil];
        NSString *str = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"30切换联系方式可见状态%@",str);
        
        aSuccess(responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        
        NSLog(@"30切换联系方式可见状态%@",error);
    }];
}
#pragma mark --31查询提现记录
+(void)upDataTiXianJiLusuccess:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/user/app_qryDrawMoneyRecord.action",SER_VICE];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    [dic setObject:[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]] forKey:@"account"];
    ...
    [manager POST:urlStr parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        NSLog(@"31查询提现记录%@",error);
    }];

}

#pragma mark --32查询用户隐藏状态
+(void)upDataYinCangStypesuccess:(SuccessBlock)aSuccess error:(ErrorBlock)aError{
    NSString * urlStr =[NSString stringWithFormat:@"%@/user/app_qryShowConnecttelStatus.action",SER_VICE];
    NSMutableDictionary * dic =[NSMutableDictionary new];
    [dic setObject:[ToolClass isString:[NSString stringWithFormat:@"%@",[NSUSE_DEFO objectForKey:@"username"]]] forKey:@"account"];
    ...
    [manager POST:urlStr parameters:dic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSData *data = [NSJSONSerialization dataWithJSONObject:responseObject options:NSJSONWritingPrettyPrinted error:nil];
        NSString *str = [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
        NSLog(@"32查询用户隐藏状态%@",str);
        
        aSuccess(responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
        
        NSLog(@"32查询用户隐藏状态%@",error);
    }];
}
@end
