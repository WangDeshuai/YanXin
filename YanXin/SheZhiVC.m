...
-(void)tuiChuBtn{
    ...
    UIAlertAction * action1 =[UIAlertAction actionWithTitle:@"是" style:0 handler:^(UIAlertAction * _Nonnull action) {
        [NSUSE_DEFO removeObjectForKey:@"有个人简介"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            //联系我们
            UIAlertController * actionView =[UIAlertController alertControllerWithTitle:@"" message:@"拨打客服" preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction * action1 =[UIAlertAction actionWithTitle:@"是" style:0 handler:^(UIAlertAction * _Nonnull action) {
