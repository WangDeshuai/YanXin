...
-(void)CreatData{
    if ([ToolClass isLogin]==NO || [[NSUSE_DEFO objectForKey:@"username"] isEqualToString:@"15032735032"]) {
        NSArray * arr1 =@[@"我的主页"];
        NSArray * arr2 =@[@"邀请分享"];
        NSArray * arr3 =@[@"设置",@"说明"];
...
-(UIView*)CreatTableHead:(MineModel*)md{
   ...
   ;//181 158
    ...
    for (int i =0; i<2; i++) {
        ...
        button.sd_layout
        .leftSpaceToView(view2,30+ScreenWidth/2*i)
    ...
    if ([ToolClass isLogin]==NO) {
        ...
        if (indexPath.section==0) {
            ...
            if (indexPath.row==0) {
                ...
                if ([[NSUSE_DEFO objectForKey:@"username"] isEqualToString:@"15032735032"]) {
                    [LCProgressHUD showMessage:@"暂无分享"];
                }else{
                ...
                //邀请分享
                 [UMSocialData defaultData].extConfig.title = @"演信";
