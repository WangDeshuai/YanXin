...
-(UIView*)CraetTabelViewHead{
    ...
    [caiFuBtn addTarget:self action:@selector(MingXi) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark --财富明细
-(void)MingXi{
    CaiFuMingXiVC * vc =[CaiFuMingXiVC new];
...
//¥
    ...
    _priceText.placeholder=@"请输入提取积分";
...
-(void)tixianButton{
    ...
    NSLog(@"输出日期%@",DateTime);
    if ([DateTime isEqualToString:@"05"] || [DateTime isEqualToString:@"15"] ||[DateTime isEqualToString:@"25"]) {
        //支付宝账号
